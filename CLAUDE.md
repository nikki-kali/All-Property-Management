...
- `properties` — listings; `is_published` gates public-site visibility. `rental_term` and `rate` are dual-purpose (rate = rent or sale price depending on context). `rental_term` has three values, not two: `short_term`, `long_term`, and `flexible` (units Katy will rent either way). `GET /api/public/properties?rental_term=...` — which powers the Long Term / Short Term toggle on the public Rentals page — matches `flexible` properties under *either* filter value (`rental_term IN ($1, 'flexible')` in `publicController.js`), not just an exact match, so a flexible unit correctly shows up on both tabs instead of needing to be filed under just one. `properties.amenities` is `JSONB` (not a Postgres `TEXT[]`) specifically so writes can go through `JSON.stringify()` the same way `titling_jobs.checklist` already does — that's what lets the same controller code work unchanged against both Postgres and the SQLite fallback (SQLite stores it as TEXT and `autoParseJSON` transparently parses it back into an array on read, same mechanism as `reminder_schedule`/`checklist`). `properties.max_occupancy` is a plain nullable `INT`. Both are edited from the admin Property Detail page (a small "Guest Capacity & Amenities" form, separate from the create-only `PropertyFormModal`) and rendered on the public site's `PropertyDetailModal` (see Frontend notes below).
...
- `/`, `/rentals`, `/buy-sell`, `/renovations`, `/titling`, `/agents`, `/about`, `/contact`, `/privacy-policy`, `/terms-of-service`, `/rebooking-refund-policy` — public marketing site, wrapped in `components/public/PublicLayout.jsx` (header/nav + footer). Pages live in `pages/public/`, shared building blocks (`Hero`, `LeadForm`, `PropertyGrid`, `SocialProof`, `SectionHeading`, `StatBlock`, `PropertyDetailModal`) in `components/public/`.

`PropertyGrid` owns the click-to-open listing-detail modal (`PropertyDetailModal`) for whatever it renders — both `RentalsPage` and `BuySellPage` get it for free since they both go through `PropertyGrid`/`PropertyCard`, rather than each page wiring up its own modal state. The modal's photo carousel reads `property.photos` (the *full* array `GET /api/public/properties` now returns per property, fetched as a second `WHERE property_id IN (...)` query rather than a JOIN or `json_agg` — same row-multiplication concern as the renovations-portfolio query below, avoided the same way), separate from `photo_url` (still just the first photo, kept for `PropertyCard`'s grid thumbnail). A property with zero/one photos just skips the arrows and dot indicators.
